...
struct ContentView: View {
    ...
    @State private var editingEntry: RainlogEntry?
...
struct EntryEditView: View {
    ...
    init(entry: RainlogEntry?, onSave: @escaping (RainlogEntry) -> Void) {
        self.entry = entry
        self.onSave = onSave
        _title = State(initialValue: entry?.title ?? "")
        _date = State(initialValue: entry?.date ?? Date())
        _f1 = State(initialValue: entry?.inches ?? "")
